import room1 from "@/assets/room-1.jpg";
import room2 from "@/assets/room-2.jpg";
import room3 from "@/assets/room-3.jpg";

export const rooms = [
  {
    id: "skyline-stay",
    title: "Skyline Stay - Single Room",
    tag: "Girls PG",
    type: "PG",
    gender: "Girls",
    price: 6500,
    distance: "1.2 km away",
    distanceKm: 1.2,
    location: "Gulmohar Colony, Bhopal",
    city: "Bhopal",
    address: "12, Gulmohar Colony, Near LNCT, Bhopal - 462042",
    description:
      "Bright, fully furnished single room in a quiet residential lane. Walking distance from LNCT and DB Mall. Home-cooked meals, hot water 24/7, and a friendly warden.",
    furnished: true,
    amenities: ["WiFi", "AC", "CCTV", "Hot Water", "Laundry", "Power Backup"],
    images: [room1, room3, room2],
    owner: {
      name: "Sunita Sharma",
      phone: "919876543210",
      verified: true,
      rating: 4.8,
      since: "2021",
    },
    coords: { x: 32, y: 44 },
  },
  {
    id: "comfort-living",
    title: "Comfort Living PG",
    tag: "Boys Hostel",
    type: "Hostel",
    gender: "Boys",
    price: 4200,
    distance: "400 m away",
    distanceKm: 0.4,
    location: "Indrapuri Sector C, Bhopal",
    city: "Bhopal",
    address: "Sector C, Indrapuri, Bhopal - 462022",
    description:
      "Affordable shared accommodation for students with mess facility. Four-bed dorm rooms include personal storage, a study desk, and daily meals.",
    furnished: true,
    amenities: ["Mess Included", "Parking", "WiFi", "Study Room", "Common TV"],
    images: [room2, room1, room3],
    owner: {
      name: "Rakesh Verma",
      phone: "919812345670",
      verified: true,
      rating: 4.5,
      since: "2019",
    },
    coords: { x: 58, y: 30 },
  },
  {
    id: "elite-studio",
    title: "Elite Studio Apartment",
    tag: "1BHK Flat",
    type: "Flat",
    gender: "Co-ed",
    price: 12000,
    distance: "2.5 km away",
    distanceKm: 2.5,
    location: "Arera Colony, Bhopal",
    city: "Bhopal",
    address: "E-8 Extension, Arera Colony, Bhopal - 462016",
    description:
      "Premium studio in Bhopal's most desirable neighbourhood. Open-plan kitchen, queen bed, work desk, and a balcony with skyline views.",
    furnished: true,
    amenities: ["Kitchen", "Lift", "No Broker", "AC", "Geyser", "Parking", "Balcony"],
    images: [room3, room1, room2],
    owner: {
      name: "Anil Mehta",
      phone: "919900112233",
      verified: true,
      rating: 4.9,
      since: "2020",
    },
    coords: { x: 72, y: 62 },
  },
  {
    id: "campus-corner",
    title: "Campus Corner - Twin Sharing",
    tag: "Girls PG",
    type: "PG",
    gender: "Girls",
    price: 5500,
    distance: "800 m away",
    distanceKm: 0.8,
    location: "Kolar Road, Bhopal",
    city: "Bhopal",
    address: "Kolar Road, Bhopal - 462042",
    description:
      "Twin-sharing rooms with attached bath, hot water, and AC. Close to colleges and bus stops. Owner stays on premises.",
    furnished: true,
    amenities: ["WiFi", "AC", "Attached Bath", "Mess"],
    images: [room1, room2, room3],
    owner: {
      name: "Geeta Pandey",
      phone: "919871122334",
      verified: true,
      rating: 4.6,
      since: "2022",
    },
    coords: { x: 44, y: 70 },
  },
  {
    id: "metro-nest",
    title: "Metro Nest - 2BHK Flat",
    tag: "2BHK Flat",
    type: "Flat",
    gender: "Co-ed",
    price: 18000,
    distance: "3.1 km away",
    distanceKm: 3.1,
    location: "MP Nagar Zone 2, Bhopal",
    city: "Bhopal",
    address: "MP Nagar Zone 2, Bhopal - 462011",
    description:
      "Spacious 2BHK in central MP Nagar. Modular kitchen, two balconies, family-friendly society, and flexible move-in dates.",
    furnished: false,
    amenities: ["Kitchen", "Lift", "Parking", "Pet Friendly", "Gym"],
    images: [room3, room2, room1],
    owner: {
      name: "Pooja Iyer",
      phone: "919811223344",
      verified: false,
      rating: 4.3,
      since: "2023",
    },
    coords: { x: 22, y: 22 },
  },
  {
    id: "scholars-den",
    title: "Scholars' Den Hostel",
    tag: "Boys Hostel",
    type: "Hostel",
    gender: "Boys",
    price: 3800,
    distance: "1.8 km away",
    distanceKm: 1.8,
    location: "Habibganj, Bhopal",
    city: "Bhopal",
    address: "Habibganj, Bhopal - 462024",
    description:
      "Budget-friendly hostel for students. Triple sharing with locker, study table, WiFi, and clean common spaces.",
    furnished: true,
    amenities: ["WiFi", "Mess", "Study Room", "CCTV"],
    images: [room2, room3, room1],
    owner: {
      name: "Mohit Singh",
      phone: "919812340099",
      verified: true,
      rating: 4.2,
      since: "2018",
    },
    coords: { x: 64, y: 84 },
  },
];

export function getRoom(id) {
  return rooms.find((room) => room.id === id);
}
